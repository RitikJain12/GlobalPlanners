...
int main(int argc, char* argv[]) {
  ...
  map.info.width =
      static_cast<int>(ceil(map_width / map.info.resolution));  // in cells
  map.info.height =
      static_cast<int>(ceil(map_height / map.info.resolution));  // in cells

  std::shared_ptr<Map> map_ptr =
      std::make_shared<Map>(map_width, map_height, map.info.resolution);

  std::vector<Point> left_wall = {Point(0.0, 0.0), Point(0.0, 16.5),
                                  Point(0.1, 16.5), Point(0.1, 0.0)};
  map_ptr->setObstacles(left_wall);

  std::vector<Point> bottom_wall = {Point(0.0, 0.0), Point(2.6, 0.0),
                                    Point(2.6, 0.1), Point(0.0, 0.1)};
  map_ptr->setObstacles(bottom_wall);

  std::vector<Point> right_wall = {Point(2.6, 0.0), Point(2.6, 16.5),
                                   Point(2.7, 16.5), Point(2.7, 0.0)};
