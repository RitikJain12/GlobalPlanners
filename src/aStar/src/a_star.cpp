...
AStar::AStar(std::shared_ptr<Map> map, const float theta_resolution,
  ...
  _node_data.reserve((unsigned long int)grid_width * grid_height *
                     _theta_resolution);  // Reserve space for nodes
  _node_position.resize(
      (unsigned long int)grid_width * grid_height * _theta_resolution,
      nullptr);  // Initialize node pointers

  Point::setLeastCount(_xy_resolution,
                       _theta_least_count);  // Set least count for Point class
}
...
bool AStar::checkCollision(Point point) {
  int index_x, index_y;
  if (!_map->getWorldtoMap(index_x, index_y, point.x, point.y)) return true;
  if (_map->getCost(index_x, index_y) != 0) return true;
...
std::vector<Point> AStar::getNeighbors(const Point& point) {
  ...
  Point neighbor(Point(point.x + _xy_resolution * cos(point.theta),
                       point.y + _xy_resolution * sin(point.theta),
                       point.theta));
  Point::normalizePoint(neighbor);
...
float AStar::calculateTravelCost(const Node& currentNode,
                                 const Node& neighborNode) {
  float distance =
      Point::euclideanDistance(currentNode.point, neighborNode.point);
  float angleDifference =
      Point::absAngleDiff(currentNode.point.theta, neighborNode.point.theta);
...
void AStar::backtrackPath(std::vector<Point>& path, Node* currentNode) {
  ...
  std::reverse(path.begin(),
               path.end());  // Reverse the path to get it from start to end
}
...
bool AStar::getPath(std::vector<Point>& path) {
  ...
  while (!openList.empty() && std::chrono::duration_cast<std::chrono::seconds>(
    ...
    for (const Point& neighborPoint : neighbors) {
      ...
      // Calculate costs
      float temp_g =
          currentNode->g + calculateTravelCost(*currentNode, *neighbor);
